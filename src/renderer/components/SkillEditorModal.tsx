...
export function SkillEditorModal({
  ...
  const handleMove = async () => {
    ...
    setError(null);
    try {
      const resp = await window.electronAPI.skillsMoveSkill({
        oldDir: skill.path,
        newDir: targetPath,
      });
      if (resp.success) {
    ...
    >
      ...
      >
          ...
          <div className="px-5 py-1.5 border-b border-border/30 flex flex-wrap gap-x-3 gap-y-1">
            {skill.argumentHint && (
              <span className="text-[11px] text-muted-foreground/50 font-mono">
                argument-hint: <span className="text-foreground/60">{skill.argumentHint}</span>
              </span>
            )}
            {skill.disableModelInvocation && (
              <span className="text-[11px] text-muted-foreground/50">
                disable-model-invocation: <span className="text-amber-400/70">true</span>
              </span>
            )}
            {skill.userInvocable === false && (
              <span className="text-[11px] text-muted-foreground/50">
                user-invocable: <span className="text-amber-400/70">false</span>
              </span>
            )}
            {skill.allowedTools && skill.allowedTools.length > 0 && (
              <span className="text-[11px] text-muted-foreground/50">
                allowed-tools:{' '}
                <span className="text-foreground/60">{skill.allowedTools.join(', ')}</span>
              </span>
            )}
            {skill.model && (
              <span className="text-[11px] text-muted-foreground/50">
                model: <span className="text-foreground/60">{skill.model}</span>
              </span>
            )}
            {skill.context && (
              <span className="text-[11px] text-muted-foreground/50">
                context: <span className="text-blue-400/70">{skill.context}</span>
              </span>
            )}
            {skill.agent && (
              <span className="text-[11px] text-muted-foreground/50">
                agent: <span className="text-foreground/60">{skill.agent}</span>
              </span>
            )}
          </div>
        )}
        ...
        <div className="flex items-center gap-2 px-5 py-3 border-t border-border/40 flex-shrink-0">
          ...
          {!confirmDelete &&
            (skill.scope === 'project' || (skill.scope === 'global' && activeProjectPath)) && (
              <button
                type="button"
                onClick={handleMove}
                disabled={moving}
